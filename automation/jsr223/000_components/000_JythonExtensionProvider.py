...
def scriptLoaded(*args):    
    try:
        ...
        class JythonExtensionProvider (ScriptExtensionProvider):            
            ...
            def getTypes(self):
                """Returns the supported types which can be received by the given ScriptExtensionProvider.
                    "Types" are just names can refer to any object, either instance or class."""
        ...
        openhab.JythonExtensionProvider = JythonExtensionProvider()
        scriptExtension.addScriptExtensionProvider(openhab.JythonExtensionProvider)
    except:
        openhab.JythonExtensionProvider = None
        import traceback
        log.error(traceback.format_exc())

def scriptUnloaded():
    if openhab.JythonExtensionProvider is not None:
        scriptExtension.removeScriptExtensionProvider(openhab.JythonExtensionProvider)
        openhab.JythonExtensionProvider = None
