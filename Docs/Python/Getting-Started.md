...
- Since the Automation API is still under development, it is best to use a current testing or snapshot release of openHAB. 
More importantly, there have been breaking changes in the API, so at least OH 2.4 snapshot 1319 (or 2.4M3) is required to use these modules.
...
- Turn on debugging for org.eclipse.smarthome.automation (`log:set DEBUG org.eclipse.smarthome.automation` in Karaf). 
Leave this on for setup and testing, but you may want to set to WARN when everything is setup. 
Newer OH builds after the ESH reintegration (2.5M2 or snapshot 1566) will require this to be changed to org.openhab.core.automation.
- Turn on debugging for jsr223 (`log:set DEBUG jsr223` in Karaf). 
This is the default logging location for all of the helper libraries in this repo.
This location can be changed by modifying the LOG_PREFIX variable in configuration.py, but it is best to not change it. 
If you change it, be sure to set the logging level for the new location.
- Shut down openHAB.
...
This will create a directory structure as described in [File Locations](#file-locations), and will include all of the Core files, including a startup delay script that ensures OH is started completely before loading other scripts.
...
- There is a main configuration file for the Jython helper libraries. 
In `\automation\lib\python\`, rename the file `configuration.py.example` to `configuration.py`. 
This can be used to secure personal information, or to store variables that are accesed by multiple scripts or modules, e.g. LOG_PREFIX. 
...
A full installation of Jython can also be used, but the paths above will need to be modified.
Jython 2.7.1 and 2.7.2a1+ will also work, but 2.7.0 has proven to be very stable.
- Copy the [`hello_world.py`](/Script%20Examples/Python/hello_world.py) example script to `/automation/jsr223/personal/` to test if everything is working. 
        ...
        └── automation
            ...
            └── lib
                └── python
                    ...
                    └── configuration.py
```
OH requires **scripts** to be located in the `/automation/jsr223/` subdirectory of your configuration directory. 
...
You can add a `-Dpython.path=mypath1:mypath2` to the JVM command line by modifying the OH startup scripts.
...
Scripts that implement OH components (like trigger types, Item providers, etc.) are one example.
To prioritze the loading of scripts, a numeric prefix can be added to the name of the script. 
There are more details about the script load order in the [OH JSR223 documentation](https://www.openhab.org/docs/configuration/jsr223.html#script-locations).
</ul>
