...
try:
    class JythonExtensionProvider(ScriptExtensionProvider):
        def __init__(self):
            logging.getLogger(LOG_PREFIX + ".core.JythonExtensionProvider").debug("Start init")
            self._defaultPresets = set()
            self._presets = set()
            self._preset_values = collections.defaultdict(list)
            self._values = {}
            logging.getLogger(LOG_PREFIX + ".core.JythonExtensionProvider").debug("End init")
    ...
    import traceback
    logging.getLogger(LOG_PREFIX + ".core.JythonExtensionProvider").warn(traceback.format_exc())

def scriptLoaded(id):
    if core.JythonExtensionProvider is not None:
        scriptExtension.addScriptExtensionProvider(core.JythonExtensionProvider)
        logging.getLogger(LOG_PREFIX + ".core.JythonExtensionProvider.scriptLoaded").debug("Registered service")

def scriptUnloaded():
    if core.JythonExtensionProvider is not None:
        scriptExtension.removeScriptExtensionProvider(core.JythonExtensionProvider)
        core.JythonExtensionProvider = None
        logging.getLogger(LOG_PREFIX + ".core.JythonExtensionProvider.scriptUnloaded").debug("Unregistered service")
