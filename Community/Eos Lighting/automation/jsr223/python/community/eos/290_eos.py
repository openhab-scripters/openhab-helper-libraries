...
def eos_rule_reinit(event):
    ...
    uninit()
    init(eos_rule_reinit, eos_rule_scene_command, eos_rule_scene_changed, eos_rule_light_update, eos_rule_level_source_update, eos_rule_motion_source_changed)


def eos_rule_scene_command(event):
    """Eos Scene received command Rule"""
    log.debug("{rule} triggered by '{name}' with scene '{scene}'".format(rule=RULE_SCENE_COMMAND_NAME, name=event.itemName, scene=event.itemCommand))
    update_scene(itemRegistry.get(event.itemName))


def eos_rule_scene_changed(event):
    """Eos Scene changed Rule"""
    log.debug("{rule} triggered by '{name}' with scene '{scene}'".format(rule=RULE_SCENE_CHANGED_NAME, name=event.itemName, scene=event.itemState))
