...
- Review the [JSR223 Jython documentation](https://www.openhab.org/docs/configuration/jsr223-jython.html).
 - Add/modify EXTRA_JAVA_OPTS. One way to do this is to add the following to your `start.sh` script, directly below the `DIRNAME` variable definition. This assumes that you will be using the standalone Jython 2.7.0 jar. For manual OH installations, add `/conf` after each of the `${DIRNAME}`.
    ```
